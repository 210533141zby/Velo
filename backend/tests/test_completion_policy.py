...
from app.services.completion.completion_service import _should_retry_after_rejection
...
class CompletionPolicyTests(unittest.TestCase):
    def test_standardized_profile_values(self) -> None:
        """验证standardized profile values相关行为是否符合预期。"""
    ...
    def test_truncate_context_uses_standardized_windows(self) -> None:
        """验证truncate上下文uses standardized windows相关行为是否符合预期。"""
        prefix = 'a' * 2000
        suffix = 'b' * 500
    ...
    def test_rejects_unsignaled_markdown_list_lead_in(self) -> None:
        """验证rejects unsignaled Markdown列表lead in相关行为是否符合预期。"""
        prefix = (
            '在我的文档中**数据记录**：\n'
    ...
    def test_allows_list_item_body_after_explicit_marker(self) -> None:
        """验证allows列表item body after explicit marker相关行为是否符合预期。"""
        prefix = '## 主要措施\n\n- '
        context = infer_completion_context(prefix, 'markdown')
    ...
    def test_paragraph_context_instruction_forbids_new_markdown_blocks(self) -> None:
        """验证paragraph上下文instruction forbids新建Markdown blocks相关行为是否符合预期。"""
    ...
    def test_trim_completion_returns_empty_for_blank_text(self) -> None:
        """验证trim补全结果returns空结果for blank文本相关行为是否符合预期。"""
    ...
    def test_trim_completion_keeps_full_first_non_empty_line(self) -> None:
        """验证trim补全结果keeps full first non空结果line相关行为是否符合预期。"""
    ...
    def test_post_process_reports_reason_for_empty_model_response(self) -> None:
        """验证post process reports原因for空结果模型响应结果相关行为是否符合预期。"""
        prefix = '北宋时期，王安石希望'
        suffix = ''
        context = infer_completion_context(prefix, 'markdown')
        self.assertEqual(
    ...
    def test_post_process_returns_empty_when_suffix_overlap_consumes_candidate(self) -> None:
        """验证post process returns空结果when suffix overlap consumes candidate相关行为是否符合预期。"""
        prefix = '人类线粒体DNA会'
        suffix = '影响能量代谢\n争议点：后续研究认为这属于极罕见例外。'
        context = infer_completion_context(prefix, 'markdown')
        candidate = '影响能量代谢'
    ...
    def test_rejects_fragmented_sentence_start_between_complete_sentences(self) -> None:
        """验证rejects fragmented sentence start between complete sentences相关行为是否符合预期。"""
    ...
    def test_rejects_unterminated_sentence_between_complete_sentences(self) -> None:
        """验证rejects unterminated sentence between complete sentences相关行为是否符合预期。"""
    ...
    def test_remove_leading_suffix_overlap_strips_repeated_suffix_prefix(self) -> None:
        """验证remove leading suffix overlap strips repeated suffix prefix相关行为是否符合预期。"""
    ...
    def test_post_process_keeps_only_non_overlapping_tail_when_candidate_repeats_suffix_prefix(self) -> None:
        """验证post process keeps only non overlapping tail when candidate repeats suffix prefix相关行为是否符合预期。"""
    ...
    def test_rejects_unsupported_quantitative_detail_not_present_in_context(self) -> None:
        """验证rejects unsupported quantitative明细not present in上下文相关行为是否符合预期。"""
        prefix = '研究者之所以重视它，是因为账册里频繁出现一个名字：沈见川。'
    ...
    def test_rejects_unfinished_bridge_tail(self) -> None:
        """验证rejects unfinished bridge tail相关行为是否符合预期。"""
        prefix = '1994年，'
    ...
    def test_allows_inline_enumeration_bridge_fragment(self) -> None:
        """验证allows inline enumeration bridge fragment相关行为是否符合预期。"""
    ...
    def test_rejects_short_non_terminal_bridge_fragment(self) -> None:
        """验证rejects short non terminal bridge fragment相关行为是否符合预期。"""
        prefix = '2014年9月，镇财政拨款180万元，用于屋顶防水、'
        suffix = '恒温库房和基础数字化设备采购；2016年又追加73万元，建成了第一间影像修复室。'
        context = infer_completion_context(prefix, 'markdown')
        candidate = '并增设了'
    ...
    def test_salvages_dangling_purpose_tail_into_enumeration_item(self) -> None:
        """验证salvages dangling purpose tail into enumeration item相关行为是否符合预期。"""
        prefix = '2014年9月，镇财政拨款180万元，用于屋顶防水、'
        suffix = '恒温库房和基础数字化设备采购；2016年又追加73万元，建成了第一间影像修复室。'
        context = infer_completion_context(prefix, 'markdown')
    ...
    def test_salvages_dangling_enumeration_conjunction(self) -> None:
        """验证salvages dangling enumeration conjunction相关行为是否符合预期。"""
        prefix = '2014年9月，镇财政拨款180万元，用于屋顶防水、'
        suffix = '恒温库房和基础数字化设备采购；2016年又追加73万元，建成了第一间影像修复室。'
        context = infer_completion_context(prefix, 'markdown')
    ...
    def test_post_process_reports_reason_for_too_short_candidate(self) -> None:
        """验证post process reports原因for too short candidate相关行为是否符合预期。"""
        prefix = '北宋时期，王安石希望'
        suffix = ''
        context = infer_completion_context(prefix, 'markdown')
        self.assertEqual(
    ...
    def test_retry_hint_for_fragmented_sentence_start(self) -> None:
        """验证retry hint for fragmented sentence start相关行为是否符合预期。"""
        self.assertIn('完整自然的句子开头', build_retry_hint('fragmented_sentence_start'))


class CompletionServiceTests(unittest.TestCase):
    def test_manual_retry_keeps_only_high_value_rejection_types(self) -> None:
        """验证手动retry keeps only high value rejection类型定义相关行为是否符合预期。"""
        self.assertTrue(_should_retry_after_rejection('unfinished_bridge_tail', 0, 'manual'))
        self.assertTrue(_should_retry_after_rejection('fragmented_sentence_start', 0, 'manual'))
        self.assertFalse(_should_retry_after_rejection('unsupported_quant_detail', 0, 'manual'))
        self.assertFalse(_should_retry_after_rejection('empty_model_response', 0, 'manual'))

    def test_auto_retry_is_disabled_for_rejection_reasons(self) -> None:
        """验证auto retry is disabled for rejection reasons相关行为是否符合预期。"""
        self.assertFalse(_should_retry_after_rejection('unfinished_bridge_tail', 0, 'auto'))
        self.assertFalse(_should_retry_after_rejection('unterminated_sentence', 0, 'auto'))


class RagPromptTests(unittest.TestCase):
    def test_builds_standard_no_context_answer(self) -> None:
        """验证builds standard no上下文答案相关行为是否符合预期。"""
        self.assertIn('没有找到足够相关的参考资料', build_no_context_answer())


class CompletionApiTests(unittest.IsolatedAsyncioTestCase):
    async def test_api_returns_reason_for_empty_prefix(self) -> None:
        """验证API returns原因for空结果prefix相关行为是否符合预期。"""
    ...
    async def test_api_passes_language_into_completion_service(self) -> None:
        """验证API passes language into补全结果服务相关行为是否符合预期。"""
    ...
    async def test_api_preserves_http_exception_status(self) -> None:
        """验证API preserves http exception状态相关行为是否符合预期。"""
        request = CompletionRequest(prefix='前文', suffix='', language='markdown', trigger_mode='manual')
