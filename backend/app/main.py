...
async def lifespan(app: FastAPI):
    ...
    if settings.AI_WARMUP_ON_STARTUP:
        try:
            await asyncio.wait_for(get_chat_model().ainvoke('请只回复“就绪”。'), timeout=20.0)
            logger.info('聊天模型预热完成', extra={'extra_data': {'event': 'chat_warmup_complete', 'model': settings.CHAT_MODEL}})
        except asyncio.TimeoutError:
            logger.warning('聊天模型预热超时，跳过启动期阻塞等待')
        except Exception:
            logger.exception('聊天模型预热失败')

        try:
            await asyncio.wait_for(run_in_threadpool(get_reranker().warmup), timeout=30.0)
            logger.info(
                'Rerank 模型预热完成',
                extra={'extra_data': {'event': 'rerank_warmup_complete', 'model': settings.RERANK_MODEL}},
            )
        except asyncio.TimeoutError:
            logger.warning('Rerank 模型预热超时，跳过启动期阻塞等待')
        except Exception:
