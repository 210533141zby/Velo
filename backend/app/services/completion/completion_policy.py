...
def get_profile(trigger_mode: str | None) -> CompletionProfile:
    ...
    自动补全和手动触发补全在长度、超时和输出约束上并不相同，
    这里把差异集中收口到 profile，避免调用层散落一堆 if/else。
    """
    return COMPLETION_PROFILES[normalize_trigger_mode(trigger_mode)]


def is_markdown_language(language: str | None) -> bool:
    """判断当前语言标记是否应按 Markdown 规则处理。

    补全策略里很多结构判断只在 Markdown 场景下成立，
    例如标题、列表和引用块的识别，因此这里单独收口这层判定。
    """
...
def truncate_context(prefix: str, suffix: str, trigger_mode: str | None) -> tuple[str, str]:
    ...
    这一步解决的是“补全该看多少上下文”而不是“模型最多能收多长输入”。
    自动补全更强调实时性，因此窗口更小；手动补全允许多看一点后文，
    以提升可插入性和句子闭合质量。
    """
...
def infer_completion_context(prefix: str, language: str | None) -> CompletionContext:
    ...
    补全策略并不只取决于语言，还取决于光标所在位置是不是标题、列表项或引用块。
    这里输出的 `block_kind` 会直接影响后续 prompt 约束，避免模型把段落补全
    写成新列表，或把标题补成一整段正文。
    """
...
def build_context_instruction(context: CompletionContext) -> str:
    ...
    这层规则的作用是把“结构边界”显式告诉模型。相比单纯依赖提示词风格，
    这种做法更容易解释，也更适合和后验规则一起形成稳定闭环。
    """
...
def build_retry_hint(previous_reject_reason: str | None) -> str:
    ...
    重试并不是简单重复请求，而是要告诉模型“上一版哪里不合格”。
    这里把常见失败类型映射成更具体的修正指令，用于第二次补全时收缩输出空间。
    """
...
def build_chat_messages(
    ...
    这里统一把系统提示、上下文窗口、结构约束、长度要求和重试提示拼成一份
    可复用的 chat/completions 输入。补全服务层不再拼接 prompt 细节，
    只负责决定何时调用和是否需要回退/重试。
    """
    profile = get_profile(trigger_mode)
