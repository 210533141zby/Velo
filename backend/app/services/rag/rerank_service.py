...
from app.services.rag.hybrid_search import extract_identifiers, is_structured_identifier_token, tokenize_for_bm25

try:
    import jieba.posseg as pseg
except Exception:  # pragma: no cover - 依赖缺失时回退到长度与标识符特征。
    pseg = None
...
def _model_directory(model_name: str) -> Path:
    ...
    目录名会对原模型名做安全化处理，保证不同模型在同一缓存根目录下可以并存，
    同时便于后续判断本地模型是否已经预下载完成。
    """
...
def _pos_flags_lookup(value: str) -> dict[str, set[str]]:
    """构建词性标记查找表。"""
    if pseg is None:
        return {}

    flags_by_token: dict[str, set[str]] = {}
    for item in pseg.cut(value):
...
def build_rerank_input(doc: Any) -> str:
    ...
    content = re.sub(r'\s+', ' ', str(raw_text if raw_text is not None else doc.page_content or '').strip())
...
class CrossEncoderReranker:
    ...
    def _resolve_model_name(self, device: str) -> str:
        ...
        if configured_model and configured_model.lower() != 'auto':
            if (
                device == 'cpu'
                and configured_model == settings.RERANK_GPU_MODEL.strip()
                and settings.RERANK_CPU_MODEL.strip()
            ):
                return settings.RERANK_CPU_MODEL.strip()
    ...
    def _instantiate_model(self, *, device: str, model_name: str):
        """按给定设备和模型名实例化 CrossEncoder。"""
        ...
        return model, model_dir, use_local_files_only
    ...
    def _load_model(self):
        ...
        with self._load_lock:
            ...
            device = self._forced_device or self._resolve_device()
            model_name = self._resolve_model_name(device)
            use_local_files_only = False
            try:
                self._model, model_dir, use_local_files_only = self._instantiate_model(
                    device=device,
                ...
                if device == 'cuda' and _looks_like_cuda_oom(exc):
                    logger.warning(
                        'Rerank GPU 显存不足，自动回退到 CPU 模型',
                        extra={
                            'extra_data': {
                                'event': 'rerank_gpu_oom_fallback_cpu',
                                'model': model_name,
                            }
                        },
                    )
    ...
    def score_documents(self, query: str, docs: Sequence[Any]) -> dict[Any, float]:
        ...
        try:
            ...
            if self._resolved_device == 'cuda' and _looks_like_cuda_oom(exc):
                logger.warning(
                    'Rerank 推理阶段显存不足，自动切换到 CPU 重试',
                    extra={
                        'extra_data': {
                            'event': 'rerank_inference_gpu_oom_fallback_cpu',
                ...
                self._resolved_model_name = self._resolve_model_name('cpu')
                model = self._load_model()
                raw_scores = model.predict(
                    pairs,
                    batch_size=min(settings.RERANK_BATCH_SIZE, 4),
                    show_progress_bar=False,
                )
            else:
                logger.exception(
