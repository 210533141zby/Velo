...
class Settings(BaseSettings):
    ...
    RAG_RETRIEVAL_MODE: str = "query_routed_keyword"
    RAG_PIPELINE_MODE: str = "paper_final"
    RAG_SIMPLE_CONTEXT_CHARS: int = 2400
    RAG_CONTEXTUAL_EMBED: bool = False
    # 上下文前缀要足够短，只起到补充检索线索的作用，不能把原始片段主体淹没掉。
    RAG_CONTEXTUAL_MAX_TOKENS: int = 96
    RAG_JUDGE_ENABLED: bool = True
    RAG_JUDGE_MAX_TOKENS: int = 220
    RAG_JUDGE_CONTEXT_CHARS: int = 2200
    RAG_DEEP_REVIEW_TOP_K: int = 5
    RAG_DEEP_REVIEW_CONTEXT_CHARS: int = 6000
    RAG_PAPER_FINAL_SOURCE_COUNT: int = 4
    RAG_PAPER_FINAL_COMPLEX_SOURCE_COUNT: int = 6
