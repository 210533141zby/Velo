...
function describeEmptyCompletionReason(reason: string | null) {
  ...
  if (reason === 'backend_unavailable') {
    return '补全后端未启动或仍在启动中';
  }
  if (reason === 'request_failed') {
    return '本次连接补全服务失败，请重试';
  }
  if (reason === 'request_timeout') {
    return '本次补全请求超时，请重试';
  }
  if (reason === 'internal_error') {
    return '补全服务内部错误，请稍后重试';
  }
  return '当前位置暂时没有合适补全';
}
...
function getEmptyCompletionTone(reason: string | null) {
  ...
  if (reason === 'request_failed' || reason === 'request_timeout' || reason === 'internal_error') {
...
async function fetchCompletion(prefix: string, suffix: string, triggerMode: CompletionMode): Promise<CompletionFetchResult> {
  ...
  try {
    ...
    if (didTimeout) {
      return { completion: '', reason: 'request_timeout', failed: true };
    }
  ...
  addProseMirrorPlugins() {
    ...
    return [
      new Plugin({
        ...
        state: {
          /**
           * 初始化为空装饰集，首次渲染时不显示幽灵文本。
           */
          init() {
            return DecorationSet.empty;
          },
          /**
           * 文档或光标变动时清空补全；有幽灵文本时生成对应装饰。
           */
          apply(transaction) {
